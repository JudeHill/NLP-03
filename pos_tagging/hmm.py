...
class HMMClassifier(BaseUnsupervisedClassifier):
    ...
    def reset(self):
        """
        Reset HMM parameters to be randomly initialised
        """
        A = torch.rand(self.num_states + 1, self.num_states + 1, device=self.device)
        A[:, 0] = 0.0
        B = torch.rand(self.num_states, self.num_obs, device=self.device)
    ...
    def train(
        ...
        """
        Train HMM using the given method, for the given amount of epochs
        """
        if method == "mle":
    ...
    def convert_log_space(self):
        """
        Converts transition and emission matrices from probability (or count)
        space to log-probability space.
        """
        if self.log_scale:
            return
    ...
    def train_EM_log(
        ...
        """
        Train a Hidden Markov Model (HMM) using the Expectation-Maximization (EM)
        algorithm (Baum-Welch)

        Each EM iteration consists of:

        **E-step (Forward-Backward algorithm)**  
        For each observation sequence, the forward-backward algorithm is used to
        compute posterior expectations of hidden states and transitions:

        - **Forward pass (alpha):**
        Computes log p(o_1, ..., o_t, z_t = i) for each time step `t` and state `i`
        using dynamic programming:
        
            α_t(j) = logsumexp_i [ α_{t-1}(i) + log A_{ij} ] + log B_j(o_t)

        - **Backward pass (β / beta):**
        Computes log p(o_{t+1}, ..., o_T | z_t = i) by propagating information
        backward from the end of the sequence:

            β_t(i) = logsumexp_j [ log A_{ij} + log B_j(o_{t+1}) + β_{t+1}(j) ]

        - **Posterior state probabilities (γ / gamma):**
        The posterior probability of being in state `i` at time `t`:

            γ_t(i) ∝ exp( α_t(i) + β_t(i) )

        - **Posterior transition probabilities (ξ / xi):**
        The posterior probability of transitioning from state `i` to `j`
        between times `t` and `t+1`:

            ξ_t(i, j) ∝ exp(
                α_t(i) + log A_{ij} + log B_j(o_{t+1}) + β_{t+1}(j)
            )


        **M-step (Parameter re-estimation)**  
        After accumulating expectations over all sequences:
        - Initial state probabilities are updated using expected state occupancy
        at time step 0.
        - Transition probabilities are updated using expected transition counts.
        - Emission probabilities are updated using expected state–observation
        counts.
        - A small smoothing constant (`epsilon`) is added before normalization.

        Parameters
        ----------
        inputs : Dataset
            Iterable dataset where each element is a dictionary containing
            `"input_ids"`, a sequence of observed symbols.
        num_iter : int, default=5
            Number of EM iterations to perform.
        initial_guesses : tuple(torch.Tensor, torch.Tensor), optional
            Optional initial guesses `(A, B)` for transition and emission
            probabilities. Expected to already be in log space if provided.
        continue_training : bool, default=False
            If True, continues training from the current model parameters.
            If False, parameters are reset unless `initial_guesses` are provided.
        """
    ...
    def train_EM_hard_log(
        ...
        """
        Train a Hidden Markov Model (HMM) using **hard EM** (also known as
        **Viterbi EM**) in log space.

        The algorithm iterates the following steps:

        **Hard E-step**
        ----------------
        - For each observation sequence, compute the single best state sequence
        using `viterbi_log`.
        - See the docstring of `viterbi_log()` for a detailed description of the
        Viterbi decoding algorithm.

        **M-step**
        ----------
        - Emission counts are updated by counting how often each state emits
        each observation along the Viterbi paths.
        - Transition counts are updated by counting transitions along the
        Viterbi paths, including transitions from the dummy start state.
        - Counts are smoothed with `epsilon` and normalized to obtain updated
        log-space transition and emission probabilities.
        ...
        num_iter : int, default=10
            Number of hard-EM iterations to perform.
        initial_guesses : tuple(torch.Tensor, torch.Tensor), optional
            Optional initial guesses `(A, B)` for transition and emission
            probabilities. Expected to already be in log space if provided.
        continue_training : bool, default=False
    ...
    def train_sEM(
        ...
        """
        Train a Hidden Markov Model (HMM) using **stochastic EM**
        in log space.

        The E-step for each sequence is identical to the one used in
        `train_EM_log`, i.e. the standard forward-backward algorithm in log
        space is applied to compute:
        - posterior state probabilities (γ)
        - posterior transition probabilities (ξ)

        See `train_EM_log` for a full description of the forward-backward equations
        and their log-space implementation.

        **Stochastic M-step / EMA updates**
        ----------------------------------
        For each sequence, expected counts are computed and used to update running
        estimates via:

            EMA ← (1 - η_k) · EMA + η_k · expected_counts

        where η_k is a learning rate provided by `eta_fn`, typically decreasing with
        the number of processed sequences. These EMA counts are then normalized to
        obtain updated transition and emission probabilities.
        ...
        eta_fn : Callable[[int], float], default=lambda k: 0.8
            Learning-rate schedule for the EMA updates. Takes the global update
            index `k` and returns a step size in (0, 1].
        initial_guesses : tuple(torch.Tensor, torch.Tensor), optional
            Optional initial guesses `(A, B)` for transition and emission
            probabilities. Expected to already be in log space if provided.
        continue_training : bool, default=False
    ...
    def viterbi(self, input_ids):
        """
        Compute the most likely hidden-state sequence for a given observation
        sequence using the Viterbi algorithm in **probability space**. 
        The implementation is fully vectorized over states at each time step.

        This method finds the single state path that maximizes the joint
        probability:

            argmax_{z_1,...,z_T} p(z_1,...,z_T, o_1,...,o_T)

        under the current HMM parameters. Dynamic programming is used to efficiently
        compute the optimal path, storing backpointers to recover the state sequence.

        Parameters
        ----------
        input_ids : Sequence[int]
            Sequence of observed symbol indices (length T).

        Returns
        -------
        path : List[int]
            The most likely hidden-state sequence of length T. State indices are
            zero-based and correspond to the rows of the emission matrix.
    ...
    def viterbi_log(self, input_ids):
        """
        Return the most likely hidden-state sequence for an observation sequence
        using the Viterbi algorithm in log space. See `viterbi` for a description of the algorithm.
        ...
        -------
        path : List[int]
            Most likely hidden-state sequence (zero-based state indices).
